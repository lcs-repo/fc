...
const flashcards: FlashcardTopics = {
  teratogens: [
...
export default function Home() {
  ...
  const [selectedTopic, setSelectedTopic] = useState<keyof FlashcardTopics | null>(null);

  const topics = Object.keys(flashcards) as Array<keyof FlashcardTopics>;

  const handleTopicSelect = (topic: keyof FlashcardTopics) => {
  ...
  const nextCard = () => {
    if (selectedTopic) {
      setCurrentCardIndex((prevIndex) => (prevIndex + 1) % flashcards[selectedTopic].length);
    }
  };
    ...
    <main className="main">
      ...
      {selectedTopic && (
        <div className="flashcard-container">
          <Flashcard
            question={flashcards[selectedTopic][currentCardIndex].question}
            answer={flashcards[selectedTopic][currentCardIndex].answer}
            onNext={nextCard}
          />
        </div>
      )}
